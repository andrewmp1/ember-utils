 /*
   ...
   In order to add 'class methods' to an ember class you need to make a second call to another mixin
   Ember.SyncClassMixin.apply(Ember.Class);
 ...
 */
Ember.SyncMixin = Ember.Mixin.create({
  created: function(){
  ...
  remove: function(){
    this.constructor._remove(this, data);
  },
...
Ember.SyncClassMixin = Ember.Mixin.create({
  _url: "",
  _plurals: {},
  ...
  _remove: function(record) {
    this.ajax(this_buildUrl(this_root(), record.get('id')), "DELETE", {
      context: this,
      success: function() {
        record.set('id', null);
      }
